...
export default Canister({
  ...
  getAccount: query([text], Result(Account, Message), (id) => {
    if (!id || typeof id !== "string") {
      return Err({ InvalidPayload: "Invalid account ID provided" });
    }
    ...
    if (account.owner.toString() !== ic.caller().toString()) {
  ...
  createAccount: update([AccountPayload], Result(Account, Message), (payload) => {
    ...
    const existingAccount = accountsStorage.get(payload.name);
    if ("Some" in existingAccount) {
      return Err({ AccountExists: `Account with name ${payload.name} already exists` });
    }
...
async function makePayment(address: Principal, amount: nat64) {
  ...
  if ("Err" in transferFeeResponse) {
    return Err({ PaymentFailed: `Failed to get transfer fee, err=${transferFeeResponse.Err}` });
  }
...
function generateCorrelationId(productId: text): nat64 {
  const correlationIdStr = `${productId}_${ic.caller().toText()}_${ic.time()}`;
  return hash(correlationIdStr);
}
